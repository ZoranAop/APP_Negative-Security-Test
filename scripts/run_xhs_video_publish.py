...
def main():
    ...
    ap.add_argument("--candidates-file", default="", help="Load video candidates from JSON file (generated by _fetch_xhs_multi.py)")
    ...
    if args.candidates_file:
        ...
        if cand_file.exists():
            ...
            if DEDUPE_FILE.exists():
                try:
                    used_ids = set(json.loads(DEDUPE_FILE.read_text(encoding="utf-8")))
                except Exception:
                    pass
            for c in candidates:
                nid = c.get("id", "")
                if nid and nid not in used_ids:
                    video_notes.append({
                        "note_id": nid,
                        "xsec_token": c.get("xsecToken", ""),
                        "title": c.get("title", ""),
                    })
            print(f"Loaded {len(video_notes)} candidates from file (filtered {len(candidates)-len(video_notes)} used)")
    
    if not video_notes:
        # Fetch from explore page
