...
def main() -> int:
    ...
    ap.add_argument("--dedupe-file", default=None,
                    help="去重档路径（默认按源自动放 state/seen_id_<source>.json，持久保留，自动跳过已抓内容）")
    ...
    default_dd = (state_dir / (f"seen_id_backpackers_f{args.fid}.json"
                               if args.source == "backpackers" else "seen_id_stock.json"))
    dedupe_path = (ROOT / args.dedupe_file) if args.dedupe_file else default_dd
    if args.reset_dedupe and dedupe_path.exists():
        dedupe_path.unlink(); print(f"[run_indonesia] 已重置去重档：{dedupe_path.name}")
    print(f"[run_indonesia] 去重档：{dedupe_path}（自动跳过已抓内容）")
