#!/usr/bin/env python3
"""
run_singapore.py — Singapore life content one-command publisher
(fetch → English caption → publish)

Workflow:
    1) Fetch    fetch_sg_life.py (herworld/eatbook/timeout/tripzilla)
               or fetch_stock_my.py --query singapore (Pexels/Pixabay stock)
    2) Caption  caption_multilang.py --langs en (Singapore English)
    3) Publish  publish_from_tokens.py (no bottom crop — editorial photos)

Usage examples:
    # Default: lifestyle articles from 4 SG sites, 5 accounts, 10 posts, English
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Singapore life one-command publisher (fetch → EN caption → publish)",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    PY = [sys.executable]
    ap.add_argument("--source", choices=["life", "stock"], default="life",
                    help="Source: life (herworld/eatbook/timeout/tripzilla) / stock (Pexels+Pixabay)")
    ...
    # life source options
    ap.add_argument("--sg-sources", default="herworld,eatbook,timeout,tripzilla",
    ...
    ap.add_argument("--query", default="singapore",
    ...
    # Singapore: no crop by default (editorial photos without watermarks)
    ...
    # control
    ap.add_argument("--workdir", default="sg_run",
    ...
    print(f"[run_singapore] source={args.source} posts={args.posts} langs={args.langs}")
    print(f"[run_singapore] raw={raw.name}  moments={moments.name}")
    ...
    default_dd = {
        "life": state_dir / "seen_sg_life.json",
        "stock": state_dir / "seen_sg_stock.json",
    }[args.source]
    dedupe_path = (ROOT / args.dedupe_file) if args.dedupe_file else default_dd
    if args.reset_dedupe and dedupe_path.exists():
        dedupe_path.unlink()
        print(f"[run_singapore] Reset dedupe: {dedupe_path.name}")
    print(f"[run_singapore] Dedupe file: {dedupe_path}")

    if args.source == "life":
        cmd = PY + [str(HERE / "fetch_sg_life.py"),
                    "--sources", args.sg_sources,
    ...
    if _run(cmd) != 0 or not raw.exists():
        print("[run_singapore] Fetch failed.", file=sys.stderr)
        return 1

    # ---- Step 2: English caption ----
    print("\n=== Step 2/3: English caption (Singapore) ===")
    ...
    if _run(cmd, env_extra={"DEFAULT_SCENE": args.default_scene}) != 0 or not moments.exists():
        print("[run_singapore] Caption rewrite failed.", file=sys.stderr)
        return 1

    # Count rows
    import csv as _csv
    rows = list(_csv.DictReader(moments.open(encoding="utf-8-sig")))
    print(f"[run_singapore] Moments ready: {len(rows)} posts")

    if args.skip_publish:
        print(f"\n[run_singapore] --skip-publish set. Material saved: {moments}")
        return 0
    ...
    if not acc.exists():
        print(f"[run_singapore] Accounts CSV not found: {acc}", file=sys.stderr)
        return 2
    ...
    if not args.yes:
        ...
        if ans not in ("y", "yes"):
            print("[run_singapore] Cancelled. Material saved:", moments)
            return 0
    ...
    if rc != 0:
        print("[run_singapore] Publish returned non-zero. Check logs and result/.", file=sys.stderr)
        return rc
    print("\n[run_singapore] Done. Report: result/publish_*.csv")
    return 0
