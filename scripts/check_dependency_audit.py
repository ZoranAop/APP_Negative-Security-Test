...
def main():
    ...
    if args.pubspec and os.path.exists(args.pubspec):
        ...
        with open(args.output_json or "/tmp/NS-10.json","w") as f: json.dump({"test_case":"NS-10","status":"SKIPPED","findings":[{"package":"N/A","category":"SKIPPED","notes":"无构建输入，无法执行依赖审计。"}],"reason":"Fail-Closed: SKIPPED → BLOCK，绝不为 PASS 或 FRAMEWORK_READY。","source":"无输入"}, f, indent=2)
    ...
    with open(args.output_json or "/tmp/NS-10.json","w") as f: json.dump(result, f, indent=2, ensure_ascii=False)
