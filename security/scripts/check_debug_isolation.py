...
def main():
    ...
    if not args.apk and not args.ipa and not args.manifest:
        with open(args.output or "results/evidence/NS-01.json","w") as f:
            json.dump({"test_case":"NS-01","status":"SKIPPED","findings":[{"note":"缺少 --apk/--ipa/--manifest 输入","severity":"SKIPPED"}],"reason":"Fail-Closed: 无输入 → SKIPPED → BLOCK"}, f, indent=2)
        print("NS-01 SKIPPED — 无构建输入（Fail-Closed → BLOCK）")
        sys.exit(1)
