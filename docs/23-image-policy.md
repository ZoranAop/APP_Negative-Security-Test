...
原始 N 张图片 URLs
  ...
  ├─ Step 5: 网格友好数量调整
  │    友好数: 1, 2, 4, 6, 9
  │    3→2, 5→4, 7→6, 8→6（向下取最近友好数）
  │    → 保障宫格布局完整性（避免不完整的最后一行）
  │
  ├─ Step 6: 质量排序
  │    按总像素数降序排列
...
| 文章 4 张图 → 3 张方向一致 | 网格调整 3→2 → 发 2 张 |
...
|------|---------|-----------|---------|------|
| 横图 Landscape | > 1.1 | 2, 4, 6 | 1×2, 2×2, 2×3 | 最通用，适合风景/科技/新闻 |
| 方图 Square | 0.9~1.1 | 4, 9 | 2×2, 3×3 | Instagram 风格，适合美食/产品 |
| 竖图 Portrait | < 0.9 | 2, 4 | 1×2, 2×2 | 适合人像/写真/时尚 |
