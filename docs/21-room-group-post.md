...
**默认行为：房间发帖不同步到广场。** 帖子仅出现在群聊房间中（`is_async=false`）。
如需同时发布到公开广场，需**显式指定** `--square` 参数。
...
## 21.5 `is_async`：广场发帖开关（默认关闭）

> **重要：房间发帖默认不同步到广场。** 这是设计决策——群组内容属于群成员私域，
> 不应自动暴露到公开广场。如需同时发广场，必须显式传 `--square` 或 `is_async=true`。
...
| `is_async=true`   | 发到房间的同时，后端**再复制一条公开帖到广场**（需 `--square`）        |

脚本默认 `is_async=false`；加 `--square` 才会同时发广场。
CSV 中也可在 `is_async` 列逐行控制（`true`/`false`），但无此列时默认 false。
